...
\end{equation}
where $\rho_{m(i)}$ is a medical school HRR fixed effect. Under this strategy, $\beta_{\text{train}}$ is identified off cardiologists who attended medical school in the same HRR but in different graduation cohorts and thus encountered different cath lab availability during their clinical rotations. The national HRR-mean cath lab share rose from roughly 0.14 in 1980 to roughly 0.39 in 2003, with substantial variation across HRRs in the timing of the rollout. The identifying assumption is that within-HRR cath lab rollout timing is uncorrelated with cohort-level shifts in other determinants of later practice intensity. We caution that cohorts entering medical school in the early 1980s and the early 2000s differ on margins beyond local cath lab proliferation, including managed care diffusion, the introduction of stenting and drug-eluting stents, and curriculum shifts; we revisit this concern in the robustness analysis below.

Results from both strategies are reported in Table~\ref{tab:imprint}. Column~(1) implements equation~\eqref{eq:cs} with destination HRR and year fixed effects. The training-imprint coefficient is 0.035 (SE 0.011), statistically distinguishable from zero at the 1\% level. A cardiologist who trained in an HRR where every hospital had a cath lab catheterizes about 3.5 percentage points more often than a cardiologist trained in an HRR with no cath labs, holding current practice HRR and year fixed. Column~(2) adds medical school HRR fixed effects per equation~\eqref{eq:origin}, raising $\hat\beta_{\text{train}}$ to 0.058 (SE 0.021). The tighter identification yields a larger coefficient, not a smaller one, suggesting that fixed origin-HRR features were attenuating the cross-sectional estimate. Once we condition on the school, the imprint comes through more cleanly. Column~(3) further imposes physician fixed effects, which absorb the training imprint by construction and preview the within-physician analysis we develop in Section~\ref{sec:dest}. We treat the within-origin estimate in column (2) as our preferred specification. The identifying variation is the difference in cath lab exposure between cardiologists from the same medical school but different graduation cohorts, which is plausibly orthogonal to cardiologist-level unobservables in a way that cross-sectional variation alone is not, since cath lab acquisition is a hospital-level decision rather than a cardiologist-level one.
...
\centering
\begin{minipage}[h]{\textwidth}
  \caption{Training imprint on residualized catheterization rate}
  \label{tab:imprint}
  \centering
...
\centering
\begin{minipage}[h]{\textwidth}
  \caption{Robustness: alternative AHA training-environment measures}
  \label{tab:aha-alt}
  \centering
